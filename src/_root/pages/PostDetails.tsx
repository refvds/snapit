...
const PostDetails = () => {
  ...
  const { data: userPosts, isLoading: isUserPostLoading } = useGetUserPosts(post?.author.$id);
    ...
    <div className='post_details-container'>
      ...
      {isLoading || !post ? (
        ...
        <div className='post_details-card'>
          <img src={post?.imageURL} alt='author' className='post_details-img' />
            ...
            <div className='flex-between w-full'>
              <Link to={`/profile/${post?.author.$id}`} className='flex items-center gap-3'>
                ...
                <div className='flex gap-1 flex-col'>
                  <p className='base-medium lg:body-bold text-light-1'>{post?.author.name}</p>
              ...
              <div className='flex-center gap-4'>
                <Link to={`/update-post/${post?.$id}`} className={`${user.id !== post?.author.$id && 'hidden'}`}>
                ...
                <Button
                  ...
                  variant='ghost'
                  className={`ost_details-delete_btn ${user.id !== post?.author.$id && 'hidden'}`}
                >
